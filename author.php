...
//$unread = intval(fun_get_message($curauth->ID, 'count', "msg_type='unread' OR msg_type='unrepm'"));
...
$tabs = array(
		'post' => __('文章','xiapistudio')."($posts_count)",
		'comment' => __('评论','xiapistudio')."($comments_count)",
		'collect' => __('收藏','xiapistudio')."($collects_count)",
		'credit' => __('积分','xiapistudio')."($credit)",
		'message' => __('消息','xiapistudio'),
		'profile' => __('资料','xiapistudio'),
);
...
$get_tab = isset($_GET['tab']) && in_array($_GET['tab'], $tab_array) ? $_GET['tab'] : 'post';
	...
	if($get_tab=='profile' && ($current_user->ID!=$curauth->ID && current_user_can('edit_users')) ) $message = sprintf(__('你正在查看的是%s的资料，修改请慎重！', 'xiapistudio'), $curauth->display_name);
	
	// 积分start
	
	if( isset($_POST['creditNonce']) && current_user_can('edit_users') ){
		if ( ! wp_verify_nonce( $_POST['creditNonce'], 'credit-nonce' ) ) {
			$message = __('安全认证失败，请重试！','xiapistudio');
		}else{
			$c_user_id =  $curauth->ID;
			if( isset($_POST['creditChange']) && sanitize_text_field($_POST['creditChange'])=='add' ){
				$c_do = 'add';
				$c_do_title = __('增加','xiapistudio');
			}else{
				$c_do = 'cut';
				$c_do_title = __('减少','xiapistudio');
			}

			$c_num =  intval($_POST['creditNum']);
			$c_desc =  sanitize_text_field($_POST['creditDesc']);
			
			$c_desc = empty($c_desc) ? '' : __('备注','xiapistudio') . ' : '. $c_desc;

			update_tin_credit( $c_user_id , $c_num , $c_do , 'tin_credit' , sprintf(__('%1$s将你的积分%2$s %3$s 分。%4$s','xiapistudio') , $current_user->display_name, $c_do_title, $c_num, $c_desc) );
			
			$message = sprintf(__('操作成功！已将%1$s的积分%2$s %3$s 分。','xiapistudio'), $user_name, $c_do_title, $c_num);
		}
	}	
	...
	if( isset($_POST['promoteVipNonce']) && current_user_can('edit_users') ){
		if ( ! wp_verify_nonce( $_POST['promoteVipNonce'], 'promotevip-nonce' ) ) {
			...
			if( isset($_POST['promotevip_type']) && sanitize_text_field($_POST['promotevip_type'])=='3' ){
				$pv_type = 3;
				$pv_type_title = __('年费会员','xiapistudio');
			}elseif(isset($_POST['promotevip_type']) && sanitize_text_field($_POST['promotevip_type'])=='2'){
				$pv_type = 2;
				$pv_type_title = __('季费会员','xiapistudio');
			}else{
				$pv_type = 1;
				$pv_type_title = __('月费会员','xiapistudio');
			}
			...
			$message = sprintf(__('操作成功！已成功将%1$s提升至%2$s，有效期至 %3$s。','xiapistudio'), $curauth->display_name, $pv_type_title, date('Y年m月d日 H时i分s秒',strtotime($pv_expire_date)));
	...
	if( isset($_POST['promoteNonce']) && current_user_can('edit_users') ){
		if ( ! wp_verify_nonce( $_POST['promoteNonce'], 'promote-nonce' ) ) {
			...
			if( isset($_POST['promote_type']) && sanitize_text_field($_POST['promote_type'])=='once' ){
				$p_type = 'once';
				$p_type_title = __('一次性','xiapistudio');
			}else{
				$p_type = 'multi';
				$p_type_title = __('可重复使用','xiapistudio');
			}
			...
			$message = sprintf(__('操作成功！已成功添加优惠码%1$s，类型：%2$s 折扣：%3$s 有效期至：%4$s。','xiapistudio'), $p_code, $p_type_title, $p_discount, date('Y年m月d日 H时i分s秒',strtotime($p_expire_date)));
		}
	}
	...
	if( isset($_POST['dpromoteNonce']) && current_user_can('edit_users') ){
		if ( ! wp_verify_nonce( $_POST['dpromoteNonce'], 'dpromote-nonce' ) ) {
			...
			$message = __('操作成功！已成功删除指定优惠码','xiapistudio');
		}		
	}
	//~ 优惠码end
	...
	if( isset($_POST['pmNonce']) && $get_pm && is_user_logged_in() ){
		if ( ! wp_verify_nonce( $_POST['pmNonce'], 'pm-nonce' ) ) {
			...
			if( add_tin_message( $curauth->ID, 'unrepm', '', $pm_title, $get_pm ) ) $message = __('发送成功！','xiapistudio');
		}
	}
...
$item_html = '<li class="tip">'.__('没有找到记录','xiapistudio').'</li>';
		...
		if($update=='info'){
			...
			if (($_FILES['file']['error'])==0&&!empty($_FILES['file'])) {
    			...
    			if(!in_array($ext, $filetype)){
    				$message = __('仅允许上传JPG、GIF、BMP、PNG图片','xiapistudio');
    			}else{
    				move_uploaded_file($tempFile,$targetFile);
    				if( !file_exists( $targetFile ) ){
	        			$message = __('图片上传失败','xiapistudio');
    				} elseif( !$imginfo=getImageInfo($targetFile) ) {
        				$message = __('图片不存在','xiapistudio');
    				} else {
        				$img = $new_file_name;
        				resize($img);
        				$message = __('头像上传成功','xiapistudio');
				...
				if ( ! is_wp_error( $update_user_id ) || $update_user_avatar ) $message = __('基本信息已更新','xiapistudio');	
			}
		}
		if($update=='info-more'){
			$update_user_id = wp_update_user( array(
			...
			if ( ! is_wp_error( $update_user_id ) ) $message = __('扩展资料已更新','xiapistudio');
		}	
		if($update=='pass'){
			$data = array();
			$data['ID'] = $user_id;
			...
			if ( ! is_wp_error( $user_id ) ) $message = __('安全信息已更新','xiapistudio');
		}
	...
	if( isset($_POST['action']) && trim($_POST['action'])=='update' && wp_verify_nonce( trim($_POST['_wpnonce']), 'check-nonce' ) ) {
		...
		if( $title && $content ){
			...
			if( mb_strlen($content,'utf8')<140 ){
				...
				$message = __('提交失败，文章内容至少140字。','xiapistudio');
				...
				if( is_wp_error( $new_post ) ){
					$message = __('操作失败，请重试或联系管理员。','xiapistudio');
				}else{
			...
			$message = __('投稿失败，标题和内容不能为空！','xiapistudio');
		}
	}
}
//~ 投稿end
...
<div id="main-wrap">
	...
	<div class="container pagewrapper clr" id="author-page">
		...
		<?php $tab_output = '';
			...
			echo '<li id="page-sort-menu-btn"><a href="#">'.__('菜单','xiapistudio').'</a></li><ul class="pagesider-menu user-tab">'.$tab_output.'</ul>';
		?>
		</aside>
		...
		<div class="content">
			<div class="user-basic-info clr">
				<a class="user-avatar" href="<?php echo esc_url( get_author_posts_url( $curauth->ID ) ) ;?>">
					<?php echo fun_get_avatar( $curauth->ID , '80' , fun_get_avatar_type($curauth->ID) ); ?>
				</a>
				<div class="user-text">
				<h4 class="user-display-name"><?php echo $user_name;?><?php if(function_exists('tin_member_icon'))echo tin_member_icon($curauth->ID); ?><?php
				if( $current_user->ID && $current_user->ID!=$curauth->ID ) echo '<small class="pm"><a href="'.add_query_arg('tab', 'message', get_author_posts_url( $curauth->ID )).'">'.__('私信TA','xiapistudio').'</a></small>';
				?></h4>
				<p class="user-register-time"><?php
				 echo date( 'Y年m月d日', strtotime( $user_info->user_registered ) ).'<span>'.__('注册','xiapistudio').'</span>';
				 if($current_user&&$current_user->ID==$curauth->ID&&!empty($user_info->tin_latest_ip_before)) {echo date( 'Y年m月d日', strtotime( $user_info->tin_latest_login_before ) ).'<span>'.__('上次登录','xiapistudio').'</span>';/*.$user_info->tin_latest_ip_before.' '.convertip($user_info->tin_latest_ip_before).'<span>'.'&nbsp;IP&nbsp;'.'</span>';*/}else{
				 if($user_info->tin_latest_login) echo date( 'Y年m月d日', strtotime( $user_info->tin_latest_login ) ).'<span>'.__('最后登录','xiapistudio').'</span>';}
				 ?></p>
				<p class="user-description"><?php 
						$description = $curauth->description;
						echo $description ? $description : __('没有个人说明','xiapistudio'); ?>
				</p>
			  </div>
			</div>
<?php
...
if( $get_tab=='credit' ) {
	...
	<div class="panel panel-danger">
		<div class="panel-heading"><?php echo $curauth->display_name.__('积分变更（仅管理员可见）','xiapistudio');?></div>
		<div class="panel-body">
			<form id="creditform" role="form"  method="post">
				...
				<p>
					<label class="radio-inline"><input type="radio" name="creditChange" value="add" aria-required='true' required checked=""><?php _e('增加积分','xiapistudio');?></label>
					<label class="radio-inline"><input type="radio" name="creditChange" value="cut" aria-required='true' required><?php _e('减少积分','xiapistudio');?></label>
				</p>
				<div class="form-inline">
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon"><?php _e('积分','xiapistudio');?></div>
							<input class="form-control" type="text" name="creditNum" aria-required='true' required>
						</div>
					</div>
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon"><?php _e('备注','xiapistudio');?></div>
							<input class="form-control" type="text" name="creditDesc" aria-required='true' required>
						</div>
					</div>
					<button class="btn btn-default" type="submit" style="margin-bottom:20px;"><?php _e('提交','xiapistudio');?></button>
				</div>
				<p class="help-block"><?php _e('请谨慎操作！积分数只能填写数字，备注将显示在用户的积分记录中。','xiapistudio');?></p>
			</form>
		</div>
	</div>
	...
	<div class="panel panel-success">
		<div class="panel-heading"><?php echo __('积分充值（仅自己可见）','xiapistudio');?></div>
		<div class="panel-body">
			<form id="creditrechargeform" role="form"  method="post" action="<?php echo THEME_URI.'/functions/alipay/alipayapi.php'; ?>" onsubmit="return false;">
				...
				<p>
					<label><?php echo sprintf(__('当前积分兑换比率为：1元 = %1$s 积分','xiapistudio'),ot_get_option('tin_cash_credit_ratio',50));?></label>
				</p>
				<div class="form-inline">
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon"><?php _e('积分*100','xiapistudio');?></div>
							<input class="form-control" type="text" name="creditrechargeNum" value="10" aria-required='true' required>
						</div>
					</div>
					<button class="btn btn-default" type="submit" id="creditrechargesubmit" style="margin-bottom:20px;"><?php _e('充值','xiapistudio');?></button>
				</div>
				<p class="help-block"><?php _e('积分数以100为单位起计算,请填写整数数值，如填1即表明充值100积分，所需现金根据具体兑换比率计算。','xiapistudio');?></p>
			</form>
		</div>
	</div>
	...
	$item_html = '<li class="tip">' . sprintf(__('共有 %1$s 个积分，其中 %2$s 个已消费， %3$s 个可用。','xiapistudio'), ($credit+$credit_void), $credit_void, $credit) ;
	if($current_user->ID==$curauth->ID){$item_html .= '&nbsp;(&nbsp;每日签到：'.tin_whether_signed($current_user->ID).'&nbsp;)';}
	$item_html .= '</li>';

	if($oneself){
		$all = fun_get_message($curauth->ID, 'count', "msg_type='credit'");
		$pages = ceil($all/$number);
		
		$creditLog = fun_get_credit_message($curauth->ID, $number,$offset);
		...
		if($creditLog){
			...
			if($pages>1) $item_html .= '<li class="tip">' . sprintf(__('第 %1$s 页，共 %2$s 页，每页显示 %3$s 条。','xiapistudio'),$paged, $pages, $number). '</li>';
		}
	}
	
	echo '<ul class="user-msg">'.$item_html.'</ul>';
	
	if($oneself) echo fun_pager($paged, $pages);

	?>
    <table class="table table-bordered credit-table">
      <thead>
        <tr class="active">
          <th><?php _e('积分方法','xiapistudio');?></th>
          <th><?php _e('一次得分','xiapistudio');?></th>
          <th><?php _e('可用次数','xiapistudio');?></th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><?php _e('注册奖励','xiapistudio');?></td>
          <td><?php printf( __('%1$s 分','xiapistudio'), ot_get_option('tin_reg_credit','50'));?></td>
          <td><?php _e('只有 1 次','xiapistudio');?></td>
        </tr>
        <tr>
          <td><?php _e('文章投稿','xiapistudio');?></td>
          <td><?php printf( __('%1$s 分','xiapistudio'), ot_get_option('tin_rec_post_credit','50'));?></td>
          <td><?php printf( __('每天 %1$s 次','xiapistudio'), ot_get_option('tin_rec_post_num','5'));?></td>
        </tr>
        <tr>
          <td><?php _e('评论回复','xiapistudio');?></td>
          <td><?php printf( __('%1$s 分','xiapistudio'), ot_get_option('tin_rec_comment_credit','5'));?></td>
          <td><?php printf( __('每天 %1$s 次','xiapistudio'), ot_get_option('tin_rec_comment_num','50'));?></td>
        </tr>
        <tr>
          <td><?php _e('访问推广','xiapistudio');?></td>
          <td><?php printf( __('%1$s 分','xiapistudio'), ot_get_option('tin_rec_view_credit','5'));?></td>
          <td><?php printf( __('每天 %1$s 次','xiapistudio'), ot_get_option('tin_rec_view_num','50'));?></td>
        </tr>
        <tr>
          <td><?php _e('注册推广','xiapistudio');?></td>
          <td><?php printf( __('%1$s 分','xiapistudio'), ot_get_option('tin_rec_reg_credit','50'));?></td>
          <td><?php printf( __('每天 %1$s 次','xiapistudio'), ot_get_option('tin_rec_reg_num','5'));?></td>
        </tr>
        <tr>
          <td><?php _e('文章互动','xiapistudio');?></td>
          <td><?php printf( __('%1$s 分','xiapistudio'), ot_get_option('tin_like_article_credit','5'));?></td>
          <td><?php printf( __('每天 %1$s 次','xiapistudio'), ot_get_option('tin_like_article_credit_times','5'));?></td>
        </tr>
        <tr>
          <td><?php _e('每日签到','xiapistudio');?></td>
          <td><?php printf( __('%1$s 分','xiapistudio'), ot_get_option('tin_daily_sign_credits','10'));?></td>
          <td><?php _e('每天 1 次','xiapistudio');?></td>
        </tr>
		<tr>
          <td><?php _e('发布资源','xiapistudio');?></td>
          <td><?php printf( __('%1$s 分','xiapistudio'), ot_get_option('tin_rec_resource_dl_credit','5'));?></td>
          <td><?php _e('不限次数,收费资源额外返还价格100%积分','xiapistudio');?></td>
        </tr>
		<?php if(ot_get_option('shop_system')&&ot_get_option('shop_system')=='on'){ ?>
		<tr>
          <td><?php _e('积分兑换','xiapistudio');?></td>
          <td colspan="2"><?php printf( __('兑换比率：1 元 = %1$s 积分','xiapistudio'), ot_get_option('tin_cash_credit_ratio','50'));?></td>
        </tr>
		<?php } ?>
      </tbody>
    </table>
...
if( $get_tab=='post' ) {
	...
	if( isset($_GET['action']) && in_array($_GET['action'], array('new', 'edit')) && $cat_count && is_user_logged_in() && $oneself && current_user_can('edit_posts') ){
		echo '<ul class="user-msg"><li class="tip">'.__('请发表你自己的文章','xiapistudio').'</ul></li>';
...
?>
			<article class="panel panel-default archive" role="main">
					<div class="panel-body">
						<h3 class="page-header"><?php _e('投稿','xiapistudio');?> <small><?php _e('POST NEW','xiapistudio');?></small></h3>
						<form role="form" method="post">
							<div class="form-group">
								<input type="text" class="form-control" name="post_title" placeholder="<?php _e('在此输入标题','xiapistudio');?>" value="<?php echo $post_title;?>" aria-required='true' required>
							</div>
							<div class="form-group">
						...
						<?php
							...
							if($can_post_cat){
								$post_cat_output = '<p class="help-block">'.__('选择文章分类', 'xiapistudio').'</p>';
						...
						?>
							...
							<div class="form-group">
								<p class="help-block"><?php _e('文章来源或版权信息，{link}代表文章链接，{title}代表文章标题，{url}代表本站首页地址，{name}代表本站名称', 'xiapistudio');?></p>
								<?php
								$cc = '';
								if(isset($_GET['id'])) $cc = get_post_meta( intval($_GET['id']), 'tin_copyright_content', true );
								$copyright_content = $cc ? $cc : ot_get_option('tin_copyright_content_default',sprintf(__('<p>原文链接：%s，转发请注明来源！</p>','xiapistudio'),'<a href="{link}" rel="author">{title}</a>'));
								?>
							...
							<div class="form-group text-right">
								<select name="post_status">
									<option value ="pending"><?php _e('提交审核','xiapistudio');?></option>
									<option value ="draft"><?php _e('保存草稿','xiapistudio');?></option>
								</select>
								<input type="hidden" name="action" value="update">
								<input type="hidden" id="_wpnonce" name="_wpnonce" value="<?php echo wp_create_nonce( 'check-nonce' );?>">
								<button type="submit" class="btn btn-success"><?php _e('确认操作','xiapistudio');?></button>
							</div>	
						</form>
					</div>
			 </article>
<?php
		...
		if($cat_count){
			...
			$item_html = sprintf( __('现有%s个分类接受投稿。', 'xiapistudio'), $cat_count );
			
			if( is_user_logged_in() && !current_user_can('edit_posts') ){
					
					$item_html .= __('遗憾的是，你现在登录的账号没有投稿权限！', 'xiapistudio');
					
				}else{
					
					$item_html .= '<a href="'.( is_user_logged_in() ? add_query_arg(array('tab'=>'post','action'=>'new'), get_author_posts_url($current_user->ID)) : wp_login_url() ).'">'.__('点击投稿', 'xiapistudio').'</a>';
			}
		}else{

			if( have_posts() ) $item_html = sprintf( __('发表了 %s 篇文章', 'xiapistudio'), $posts_count );
		}
...
if( $get_tab=='comment' ) {
	...
	if($comments){
		$item_html = '<li class="tip">' . sprintf(__('共有 %1$s 条评论，其中 %2$s 条已获准， %3$s 条正等待审核。','xiapistudio'),$all, $approve, $all-$approve) . '</li>';
		foreach( $comments as $comment ){
			$item_html .= ' <li>';
			if($comment->comment_approved!=1) $item_html .= '<small class="text-danger">'.__( '这条评论正在等待审核','xiapistudio' ).'</small>';
			$item_html .= '<div class="message-content">'.$comment->comment_content . '</div>';
			$item_html .= '<a class="info" href="'.htmlspecialchars( get_comment_link( $comment->comment_ID) ).'">'.sprintf(__('%1$s  发表在  %2$s','xiapistudio'),$comment->comment_date,get_the_title($comment->comment_post_ID)).'</a>';
			$item_html .= '</li>';
		}
		if($pages>1) $item_html .= '<li class="tip">'.sprintf(__('第 %1$s 页，共 %2$s 页，每页显示 %3$s 条。','xiapistudio'),$paged, $pages, $number).'</li>';
	}
	...
	echo '<ul class="user-msg">'.$item_html.'</ul>';
	echo fun_pager($paged, $pages);
}
//~ 评论end

// 收藏start
if( $get_tab=='collect'){
	$item_html = '<li class="tip">'.__('共收藏了','xiapistudio').$collects_count.'篇文章</li>';
	echo '<ul class="user-msg"><li class="tip">'.$item_html.'</ul></li>';
	//global $wp_query;
...
<div class="page-header">
	<h3 id="membership-info"><?php _e('会员信息','xiapistudio'); ?></h3>
</div>
...
<?php if(current_user_can('edit_users')){ ?>
	<div class="panel panel-danger">
		<div class="panel-heading"><?php echo __('会员操作（本选项卡及内容仅管理员可见）','xiapistudio');?></div>
		<div class="panel-body">
			<form id="promotevipform" role="form"  method="post">
				...
				<p>
					<label class="radio-inline"><input type="radio" name="promotevip_type" value="1" aria-required='true' required checked><?php _e('月费会员','xiapistudio');?></label>
					<label class="radio-inline"><input type="radio" name="promotevip_type" value="2" aria-required='true' required><?php _e('季费会员','xiapistudio');?></label>
					<label class="radio-inline"><input type="radio" name="promotevip_type" value="3" aria-required='true' required><?php _e('年费会员','xiapistudio');?></label>
				</p>
				<div class="form-inline">
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon"><?php _e('会员截止有效期','xiapistudio');?></div>
							<input class="form-control" type="date" name="vip_expire_date" aria-required='true' required>
						</div>
					</div>
					<button class="btn btn-default" id="promotevipform-submit" type="submit" style="margin-bottom:20px;"><?php _e('确认操作','xiapistudio');?></button>
				</div>
				<p class="help-block"><?php _e('请谨慎操作！会员截止有效期格式2015-01-01','xiapistudio');?></p>
			</form>
		</div>
	</div>
<?php } ?>
...
-->
<div class="page-header">
	<h3 id="membership-join"><?php _e('加入会员','xiapistudio'); ?> <small><?php _e('加入、续费','xiapistudio'); ?></small></h3>
</div>
<div class="panel">
	<div class="panel-body">
		<form id="joinvip" role="form" method="post" action="<?php echo THEME_URI.'/functions/alipay/alipayapi.php'; ?>" onsubmit="return false;">
			<p>
				...
				<input type = "hidden" id="order_id" name="order_id" readonly="" value="0">
				<label class="radio-inline"><input type="radio" name="product_id" value="-1" aria-required="true" required="" checked=""><?php _e('月费会员','xiapistudio'); ?>(<?php echo ot_get_option('vip_monthly_price',5).'元/月'; ?>)</label>
				<label class="radio-inline"><input type="radio" name="product_id" value="-2" aria-required="true" required=""><?php _e('季费会员','xiapistudio'); ?>(<?php echo ot_get_option('vip_quarterly_price',12).'元/季'; ?>)</label>
				<label class="radio-inline"><input type="radio" name="product_id" value="-3" aria-required="true" required=""><?php _e('年费会员','xiapistudio'); ?>(<?php echo ot_get_option('vip_annual_price',45).'元/年'; ?>)</label>
				<button class="btn btn-primary" id="joinvip-submit" type=""><?php _e('确认开通','xiapistudio'); ?></button>
			</p>
			<p class="help-block" style="font-size:12px;"><?php _e('提示:若已开通会员则按照选择开通的类型自动续费,若会员已到期,则按重新开通计算有效期','xiapistudio'); ?></p>
		</form>
	</div>
...
<div class="page-header">
	<h3 id="membership-records"><?php _e('会员记录','xiapistudio'); ?> <small><?php _e('会员订单','xiapistudio'); ?></small></h3>
</div>
<div class="wrapbox">
		<div class="membership-history order-history">
			<table width="100%" border="0" cellspacing="0">
				<thead>
					<tr>
						<th scope="col"><?php _e('订单号','xiapistudio'); ?></th>
						<th scope="col"><?php _e('支付时间','xiapistudio'); ?></th>
						<th scope="col"><?php _e('支付金额','xiapistudio'); ?></th>
						<th scope="col"><?php _e('开通类型','xiapistudio'); ?></th>	
						<th scope="col"><?php _e('交易状态','xiapistudio'); ?></th>
					</tr>
				</thead>
...
<div class="shop" style="margin-top: 50px;">
	<div id="history" class="wrapbox">
		...
		<li class="contextual" style="background:#ceface;color:#44a042;"><?php echo sprintf(__('与 %1$s 相关订单记录（该栏目仅自己和管理员可见）。','xiapistudio'), $curauth->display_name); ?></li>
		<div class="pay-history">
			<table width="100%" border="0" cellspacing="0" class="table table-bordered orders-table">
				<thead>
					<tr>
						<th scope="col" style="width:20%;"><?php _e('商品名','xiapistudio'); ?></th>
						<th scope="col"><?php _e('订单号','xiapistudio'); ?></th>
						<th scope="col"><?php _e('购买时间','xiapistudio'); ?></th>
						<th scope="col"><?php _e('数量','xiapistudio'); ?></th>
						<th scope="col"><?php _e('价格','xiapistudio'); ?></th>
						<th scope="col"><?php _e('总价','xiapistudio'); ?></th>
						<th scope="col"><?php _e('交易状态','xiapistudio'); ?></th>
					</tr>
				</thead>
				<tbody class="the-list">
...
<?php
	...
	if($oLog){
		$item_html = '<li class="contextual" style="background:#f2dede;color:#a94442;">' . sprintf(__('全站共有 %1$s 条订单记录（该栏目仅管理员可见）。','xiapistudio'), $oall) . '</li>';
		$item_html .= '<div class="site-orders">
			<table width="100%" border="0" cellspacing="0" class="table table-bordered orders-table">
				<thead>
					<tr>
						<th scope="col" style="width:20%;">'.__('商品名','xiapistudio').'</th>
						<th scope="col">'.__('订单号','xiapistudio').'</th>
						<th scope="col">'.__('买家','xiapistudio').'</th>
						<th scope="col">'.__('购买时间','xiapistudio').'</th>
						<th scope="col">'.__('总价','xiapistudio').'</th>
						<th scope="col">'.__('交易状态','xiapistudio').'</th>
						<th scope="col">'.__('操作','xiapistudio').'</th>
					</tr>
				</thead>
				<tbody class="the-list">';
				foreach($oLog as $Log){
...
?>
	<div class="panel panel-danger">
		<div class="panel-heading"><?php echo __('添加优惠码（本选项卡及内容仅管理员可见）','xiapistudio');?></div>
		<div class="panel-body">
			<form id="promoteform" role="form"  method="post">
				...
				<p>
					<label class="radio-inline"><input type="radio" name="promote_type" value="once" aria-required='true' required checked><?php _e('一次性','xiapistudio');?></label>
					<label class="radio-inline"><input type="radio" name="promote_type" value="multi" aria-required='true' required><?php _e('重复使用','xiapistudio');?></label>
				</p>
				<div class="form-inline">
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon"><?php _e('优惠码','xiapistudio');?></div>
							<input class="form-control" type="text" name="promote_code" aria-required='true' required>
						</div>
					</div>
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon"><?php _e('折扣','xiapistudio');?></div>
							<input class="form-control" type="text" name="discount_value" aria-required='true' required>
						</div>
					</div>
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon"><?php _e('截止有效期','xiapistudio');?></div>
							<input class="form-control" type="text" name="expire_date" aria-required='true' required>
						</div>
					</div>
					<button class="btn btn-default" type="submit" style="margin-bottom:20px;"><?php _e('添加','xiapistudio');?></button>
				</div>
				<p class="help-block"><?php _e('请谨慎操作！折扣只能填写0~1之间的数字并精确到2位小数点，有效期格式2015-01-01 10:20:30。','xiapistudio');?></p>
			</form>
		</div>
	</div>
	...
	<table class="table table-bordered promote-table">
      ...
      <thead>
        <tr class="active">
          <th><?php _e('优惠码','xiapistudio');?></th>
          <th><?php _e('类型','xiapistudio');?></th>
          <th><?php _e('折扣','xiapistudio');?></th>
		  <th><?php _e('截止有效期','xiapistudio');?></th>
		  <th><?php _e('使用状态','xiapistudio');?></th>
		  <th><?php _e('操作','xiapistudio');?></th>
        </tr>
      </thead>
      <tbody>
	  ...
	  ?>
        <tr>
		  <input type="hidden" name="promote_id" value="<?php echo $pcode['id']; ?>" >
			...
			<td class="delete_promotecode"><a><?php _e('删除','xiapistudio');?></a></td>
        </tr>
	  <?php
		}
	  ?>
...
if( $get_tab=='message' ) {
	...
	if($current_user->ID==$curauth->ID){
		...
		$all = fun_get_message($curauth->ID, 'count', $all_sql);
		
		$pages = ceil($all/$number);
		

		$mLog = fun_get_message($curauth->ID, '', $all_sql, $number,$offset);

		$unread = intval(fun_get_message($curauth->ID, 'count', "msg_type='unread' OR msg_type='unrepm'"));
		
		if($mLog){
			$item_html = '<li class="tip">' . sprintf(__('共有 %1$s 条消息，其中 %2$s 条是新消息（绿色标注）。','xiapistudio'), $all, $unread) . '</li>';
			foreach( $mLog as $log ){
				$unread_tip = $unread_class = '';
				if(in_array($log->msg_type, array('unread', 'unrepm'))){
					$unread_tip = '<span class="tag">'.__('新！', 'xiapistudio').'</span>';
				...
				if(in_array($log->msg_type, array('repm', 'unrepm'))){
					...
					$msg_title = sprintf(__('%s发来的私信','xiapistudio'), $msg_title).' <a href="'.add_query_arg('tab', 'message', get_author_posts_url(intval($msg_title_data->from))).'#'.$log->msg_id.'">'.__('查看对话','xiapistudio').'</a>';
				}
		...
		if( is_user_logged_in() ){
			...
			$item_html = '<li class="tip">'.sprintf(__('与 %s 对话','xiapistudio'), $user_info->display_name).'</li><li><form id="pmform" role="form" method="post"><input type="hidden" name="pmNonce" value="'.wp_create_nonce( 'pm-nonce' ).'" ><p><textarea class="form-control" rows="3" name="pm" required></textarea></p><p class="clearfix"><a class="btn btn-link pull-left" href="'.add_query_arg('tab', 'message', get_author_posts_url($current_user->ID)).'">'.__('查看我的消息','xiapistudio').'</a><button type="submit" class="btn btn-primary pull-right">'.__('确定发送','xiapistudio').'</button></p></form></li>';
			...
			if($pmLog){
				foreach( $pmLog as $log ){
					...
					$item_html .= '<li id="'.$log->msg_id.'"><div class="message-content clearfix"><a class="'.( $pm_data->from==$current_user->ID ? 'pull-right' : 'pull-left' ).'" href="'.get_author_posts_url($pm_data->from).'">'.fun_get_avatar( $pm_data->from , '34' , fun_get_avatar_type($pm_data->from), false ).'</a><div class="pm-box"><div class="pm-content'.( $pm_data->from==$current_user->ID ? '' : ' highlight' ).'">'.htmlspecialchars_decode($log->msg_content).'</div><p class="pm-date">'.date_i18n( get_option( 'date_format' ).' '.get_option( 'time_format' ), strtotime($log->msg_date)).'</p></div></div></li>';
				}
			}
			...
			$item_html = '<li class="tip">'.sprintf(__('私信功能需要<a href="%s">登录</a>才可使用！','xiapistudio'), wp_login_url() ).'</li>';
		}
	}
	
	echo '<ul class="user-msg">'.$item_html.'</ul>'.fun_pager($paged, $pages);
...
if( $get_tab=='profile' ) {
		...
		$avatar_type = array(
			'default' => __('默认头像', 'xiapistudio'),
			'qq' => __('腾讯QQ头像', 'xiapistudio'),
			'weibo' => __('新浪微博头像', 'xiapistudio'),
			'customize' => __('自定义头像', 'xiapistudio'),
		);
		
		$author_profile = array(
			__('头像来源:','xiapistudio') => $avatar_type[fun_get_avatar_type($user_info->ID)],
			__('昵称:','xiapistudio') => $user_info->display_name,
			__('站点:','xiapistudio') => $user_info->user_url,
			__('个人说明:','xiapistudio') => $user_info->description
		);
		...
		echo '<ul class="user-msg"><li class="tip">'.sprintf(__('%s来%s已经%s天了', 'xiapistudio') , $user_info->display_name, get_bloginfo('name'), ( $days_num>1 ? $days_num : 1 ) ).'</li></ul>'.'<table id="author-profile"><tbody>'.$profile_output.'</tbody></table>';
...
<form id="info-form" class="form-horizontal" role="form" method="POST" action="">
	...
	<input type="hidden" name="_wpnonce" value="<?php echo wp_create_nonce( 'check-nonce' );?>">
			<div class="page-header">
				<h3 id="info"><?php _e('基本信息','xiapistudio');?> <small><?php _e('公开资料','xiapistudio');?></small></h>
			</div>

	<div class="form-group">
		<label class="col-sm-3 control-label"><?php _e('头像','xiapistudio');?></label>
		<div class="col-sm-9">

<div class="radio">
<?php echo fun_get_avatar( $user_info->ID , '40' , fun_get_avatar_type($user_info->ID) ); ?>
  <label>
	<input type="radio" name="avatar"  value="default" <?php if( ($avatar!='qq' || fun_is_open_qq($user_info->ID)===false) && ($avatar!='weibo' || fun_is_open_weibo($user_info->ID)===false) ) echo 'checked';?>><?php _e('默认头像','xiapistudio'); ?>
  </label>
  <label id="edit-avatar"><?php _e('(上传头像)','xiapistudio'); ?></label>
</div>

<div id="upload-input">    
    <input name="file" type="file"  value="<?php _e('浏览','xiapistudio'); ?>" >              
    <span id="upload-avatar"><?php _e('上传','xiapistudio'); ?></span>   
</div>
<p id="upload-avatar-msg"></p>

<?php if(fun_is_open_qq($user_info->ID)){ ?>
<div class="radio">
<?php echo fun_get_avatar( $user_info->ID , '40' , 'qq' ); ?>
  <label>
    <input type="radio" name="avatar" value="qq" <?php if($avatar=='qq') echo 'checked';?>> <?php _e('QQ头像', 'xiapistudio');?>
  </label>
</div>
<?php } ?>

<?php if(fun_is_open_weibo($user_info->ID)){ ?>
<div class="radio">
<?php echo fun_get_avatar( $user_info->ID , '40' , 'weibo' ); ?>
  <label>
    <input type="radio" name="avatar" value="weibo" <?php if($avatar=='weibo') echo 'checked';?>> <?php _e('微博头像', 'xiapistudio');?>
  </label>
</div>
<?php } ?>
	...
	<div class="form-group">
		<label for="display_name" class="col-sm-3 control-label"><?php _e('昵称','xiapistudio');?></label>
		<div class="col-sm-9">
	...
	<div class="form-group">
		<label for="url" class="col-sm-3 control-label"><?php _e('站点','xiapistudio');?></label>
		<div class="col-sm-9">
	...
	<div class="form-group">
		<label for="description" class="col-sm-3 control-label"><?php _e('个人说明','xiapistudio');?></label>
		<div class="col-sm-9">
	...
	<div class="form-group">
		<div class="col-sm-offset-3 col-sm-9">
			<button type="submit" class="btn btn-primary"><?php _e('保存更改','xiapistudio');?></button>
		</div>
	</div>
...
<form id="open-form" class="form-horizontal" role="form" method="post">
	...
	<?php if($qq){ ?>
		...
		<?php  if(fun_is_open_qq($user_info->ID)) { ?>
			...
			<?php echo fun_get_avatar( $user_info->ID , '100' , 'qq' ); ?>
		<?php }else{ ?>
	...
	<?php if($weibo){ ?>
		...
		<?php if(fun_is_open_weibo($user_info->ID)) { ?>
			...
			<?php echo fun_get_avatar( $user_info->ID , '100' , 'weibo' ); ?>
		<?php }else{ ?>
