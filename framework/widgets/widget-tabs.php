...
/* ------------------------------------ */
	function TinTabs() {
		parent::__construct( false, 'Tin-选项卡列表', array('description' => 'xiapistudio-以选项卡式显示文章、评论等内容', 'classname' => 'widget_tin_tabs') );;	
	}
...
/* ------------------------------------ */
	$titles = array(
		'recent'	=> $instance['title_latestpost'],//__('近期文章','xiapistudio'),
		'popular'	=> $instance['title_hotpost'],//__('热门文章','xiapistudio'),
		'comments'	=> $instance['title_comment']//__('最新评论','xiapistudio')
	);
	$tabs = array();
	$count = 0;
	$order = array(
			...
			<ul id="tab-comments" class="tin-tab group <?php if($instance['comments_avatars']) { echo 'avatars-enabled'; } else {echo 'no-avatar';} ?>">
				...
				<li>
						...
						<div class="tab-item-inner group">
							...
							<div class="tab-item-comment"><span class="arrow-poptop"></span><a href="<?php echo esc_url(get_permalink($comment->comment_post_ID)); ?>"><i><?php echo $comment->comment_author; ?></i><?php _e('说: ','xiapistudio'); ?><?php echo $comment_excerpt; ?></a><div class="tab-cmt-votes"><span class="cmt-vote">
					<?php $c_name = 'tin_comment_vote_'.$comment->comment_ID;$cookie = isset($_COOKIE[$c_name])?$_COOKIE[$c_name]:'';?>
					<i class="fa fa-thumbs-o-up <?php if($cookie==1)echo 'voted'; ?>" title="<?php _e('顶','xiapistudio'); ?>" data="<?php echo $comment->comment_ID; ?>" data-type="1" data-num="<?php echo (int)get_comment_meta($comment->comment_ID,'tin_comment_voteyes',true); ?>"><?php echo ' ['.(int)get_comment_meta($comment->comment_ID,'tin_comment_voteyes',true).']'; ?></i>
					<i class="fa fa-thumbs-o-down <?php if($cookie==2)echo 'voted'; ?>" title="<?php _e('踩','xiapistudio'); ?>" data="<?php echo $comment->comment_ID; ?>" data-type="2" data-num="<?php echo (int)get_comment_meta($comment->comment_ID,'tin_comment_voteno',true); ?>"><?php echo ' ['.(int)get_comment_meta($comment->comment_ID,'tin_comment_voteno',true).']'; ?></i>
				</span></div></div>
...
?>
	...
	<div class="tin-options-tabs">
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('标题:','xiapistudio'); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($instance["title"]); ?>" />
		</p>
		
		<h4><?php _e('近期文章','xiapistudio'); ?></h4>
		<p>
			<label for="<?php echo $this->get_field_id('title_latestpost'); ?>"><?php _e('选项卡标题,默认近期文章:','xiapistudio'); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('recent_enable'); ?>"><?php _e('启用近期文章','xiapistudio'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id('recent_thumbs'); ?>"><?php _e('显示缩略图','xiapistudio'); ?></label>
		</p>	
		<p>
			<label style="width: 55%; display: inline-block;" for="<?php echo $this->get_field_id("recent_num"); ?>"><?php _e('要显示的条目','xiapistudio'); ?></label>
		...
		<p>
			<label style="width: 100%; display: inline-block;" for="<?php echo $this->get_field_id("recent_cat_id"); ?>"><?php _e('分类:','xiapistudio'); ?></label>
		...
		<hr>
		<h4><?php _e('热门文章','xiapistudio'); ?></h4>
		<p>
			<label for="<?php echo $this->get_field_id('title_hotpost'); ?>"><?php _e('选项卡标题,默认热门文章:','xiapistudio'); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('popular_enable'); ?>"><?php _e('开启热门文章','xiapistudio'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id('popular_thumbs'); ?>"><?php _e('显示缩略图','xiapistudio'); ?></label>
		</p>	
		<p>
			<label style="width: 55%; display: inline-block;" for="<?php echo $this->get_field_id("popular_num"); ?>"><?php _e('要显示的条目','xiapistudio'); ?></label>
		...
		<p>
			<label style="width: 100%; display: inline-block;" for="<?php echo $this->get_field_id("popular_cat_id"); ?>"><?php _e('分类:','xiapistudio'); ?></label>
		...
		<p style="padding-top: 0.3em;">
			<label style="width: 100%; display: inline-block;" for="<?php echo $this->get_field_id("popular_time"); ?>"><?php _e('最热评论文章来自：','xiapistudio'); ?></label>
			<select style="width: 100%;" id="<?php echo $this->get_field_id("popular_time"); ?>" name="<?php echo $this->get_field_name("popular_time"); ?>">
			  <option value="0"<?php selected( $instance["popular_time"], "0" ); ?>><?php _e('任何时间','xiapistudio'); ?></option>
			  <option value="1 year ago"<?php selected( $instance["popular_time"], "1 year ago" ); ?>><?php _e('今年','xiapistudio'); ?></option>
			  <option value="1 month ago"<?php selected( $instance["popular_time"], "1 month ago" ); ?>><?php _e('本月','xiapistudio'); ?></option>
			  <option value="1 week ago"<?php selected( $instance["popular_time"], "1 week ago" ); ?>><?php _e('本周','xiapistudio'); ?></option>
			  <option value="1 day ago"<?php selected( $instance["popular_time"], "1 day ago" ); ?>><?php _e('过去24小时','xiapistudio'); ?></option>
			</select>	
		</p>
		
		<hr>
		<h4><?php _e('评论','xiapistudio'); ?></h4>
		<p>
			<label for="<?php echo $this->get_field_id('title_comment'); ?>"><?php _e('选项卡标题,默认最新评论:','xiapistudio'); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('comments_enable'); ?>"><?php _e('开启评论选项卡','xiapistudio'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id('comments_avatars'); ?>"><?php _e('显示头像','xiapistudio'); ?></label>
		</p>
		<p>
			<label style="width: 55%; display: inline-block;" for="<?php echo $this->get_field_id("comments_num"); ?>"><?php _e('显示的条目数量','xiapistudio'); ?></label>
		...
		<p style="padding-top: 0.3em;">
			<label style="width: 100%; display: inline-block;" for="<?php echo $this->get_field_id("comment_order"); ?>"><?php _e('评论排序','xiapistudio'); ?></label>
			<select style="width: 100%;" id="<?php echo $this->get_field_id("comment_order"); ?>" name="<?php echo $this->get_field_name("comment_order"); ?>">
			  <option value="time"<?php selected( $instance["comment_order"], "time" ); ?>><?php _e('最新评论','xiapistudio'); ?></option>
			  <option value="vote"<?php selected( $instance["comment_order"], "vote" ); ?>><?php _e('最赞评论','xiapistudio'); ?></option>
			</select>	
		</p>

		<hr>

		<h4><?php _e('Tab顺序','xiapistudio'); ?></h4>
		
		<p>
			<label style="width: 55%; display: inline-block;" for="<?php echo $this->get_field_id("order_recent"); ?>"><?php _e('近期文章','xiapistudio'); ?></label>
		...
		<p>
			<label style="width: 55%; display: inline-block;" for="<?php echo $this->get_field_id("order_popular"); ?>"><?php _e('热门文章','xiapistudio'); ?></label>
		...
		<p>
			<label style="width: 55%; display: inline-block;" for="<?php echo $this->get_field_id("order_comments"); ?>"><?php _e('最新评论','xiapistudio'); ?></label>
		...
		<hr>
		<h4><?php _e('Tab信息','xiapistudio'); ?></h4>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('tabs_category'); ?>"><?php _e('显示分类','xiapistudio'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id('tabs_date'); ?>"><?php _e('显示日期','xiapistudio'); ?></label>
		</p>
