...
// 拒绝搜索引擎索引开放平台登录地址
function fun_connect_robots_mod( $output, $public ){
	$output .= "Disallow: /?connect=*";
	return $output;
}
add_filter( 'robots_txt', 'fun_connect_robots_mod', 10, 2 );

//获取重定向链接
function fun_get_redirect_uri(){
...
//重定向Cookie名
function fun_redirect_cookie_name(){
	$home = home_url();
	$home = str_replace('.', '_', $home);
	$cookie_name = 'fun_logged_in_redirect_'.$home;
	return $cookie_name;
}

//判断QQ登录
function fun_is_open_qq($user_id=0){
	$open_qq = ot_get_option('fun_open_qq');
...
//判断微博登录
function fun_is_open_weibo($user_id=0){
	$open_weibo = ot_get_option('fun_open_weibo');
...
//判断微信登录
function fun_is_open_weixin($user_id=0){
	$open_weixin = ot_get_option('fun_open_weixin');
	if($open_weixin!='on') return false;
	$id = (int)$user_id;
	$O = array(
		'KEY'=>ot_get_option('fun_open_weixin_id'),
		'SECRET'=>ot_get_option('fun_open_weixin_key')
	);	
	if( !$O['KEY'] || !$O['SECRET'] ) return false;
	if($id){		
		$U = array(
			'ID'=>get_user_meta( $id, 'fun_weixin_openid', true ),
			'TOKEN'=>get_user_meta( $id, 'fun_weixin_access_token', true )
		);		
		if( !$U['ID'] || !$U['TOKEN'] ) return false;		
	}	
	return true;
...
function fun_login_form(){	
	...
	if( fun_is_open_weibo() ) {
		$open_data[] = array(
			'title' => __('微博','tinection'),
			'slug' => 'weibo'
		);
	}
	if( fun_is_open_weixin() ) {
		$open_data[] = array(
...
<ul class="tin-open">
	<?php
		foreach( $open_data as $open ){
			echo '<li><a class="'.$open['slug'].'" href="'.home_url('/?connect='.$open['slug'].'&action=login&redirect='.urlencode(fun_get_redirect_uri())).'" title="'.sprintf( __('使用%s账号登录', 'tinection'), $open['title']).'"></a> </li>';
		}
	?>
</ul>
	<?php
}
add_action('login_form', 'fun_login_form');

//后台个人资料QQ及微博信息
function open_user_contactmethods($user_contactmethods ){
	$user_contactmethods ['fun_qq_openid'] = 'QQ OPEN ID';
	$user_contactmethods ['fun_qq_access_token'] = 'QQ TOKEN';
	$user_contactmethods ['fun_weibo_openid'] = 'WEIBO OPEN ID';
	$user_contactmethods ['fun_weibo_access_token'] = 'WEIBO TOKEN';
...
add_action( 'profile_personal_options', 'fun_open_profile_fields' );
...
function fun_open_template_redirect(){	
		...
		if($type==='qq'){
			...
			$name = ' <img src="'.get_bloginfo('template_url').'/images/weixin_32x32.png" > 微信 ';
				...
				if( isset($_GET['wpnonce']) && wp_verify_nonce( trim($_GET['wpnonce']), $type.'_logout' ) ){
					...
					wp_die(
						sprintf( __('你正在试图解除%1$s绑定，确定这样做吗？<a href="%2$s">点击继续</a> <p>不知道这是哪里？<a href="%3$s">点击返回</a></p>','tin'), $name, add_query_arg( 'wpnonce', wp_create_nonce( $type.'_logout' ), fun_get_current_page_url() ), $redirect ),
						__('解除账号绑定','tinection')
					);	
				}	
	}
	...
	if( isset($_GET['connect']) && trim($_GET['connect'])==='qq' && fun_is_open_qq() && ( is_home() ||is_front_page() ) ){
		...
		$OPEN_QQ = array(
			'APPID'=>ot_get_option('fun_open_qq_id'),
			'APPKEY'=>ot_get_option('fun_open_qq_key'),
		...
		if( isset($_GET['action']) ){
			...
			if(trim($_GET['action'])==='logout' && is_user_logged_in()) fun_open_logout('qq');
		}
	...
	if( isset($_GET['connect']) && trim($_GET['connect'])==='weibo' && fun_is_open_weibo() && ( is_home() ||is_front_page() ) ){
		...
		if( isset($_GET['action']) ){
			...
			if(trim($_GET['action'])==='logout' && is_user_logged_in()) fun_open_logout('weibo');

		}

		if( isset($_GET['code']) ){

			$access = fun_connect_check(wp_remote_retrieve_body(wp_remote_post('https://api.weibo.com/oauth2/access_token?',array( 'body' => array(
				'grant_type'=>'authorization_code',
				'client_id'=>$OPEN_WEIBO['KEY'],
	...
	if( isset($_GET['connect']) && trim($_GET['connect'])==='weixin' && fun_is_open_weixin() && ( is_home() ||is_front_page() ) ){
		fun_set_redirect_cookie();
		$OPEN_WEIXIN = array(
			'APPID'=>ot_get_option('fun_open_weixin_id'),
			'APPKEY'=>ot_get_option('fun_open_weixin_key'),
...
add_action('template_redirect', 'fun_open_template_redirect' );
