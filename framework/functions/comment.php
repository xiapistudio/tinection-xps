...
<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
	...
	<?php echo fun_get_avatar( $comment->user_id , '54' , fun_get_avatar_type($comment->user_id) ); ?>
	...
	<div class="comment-main">
		...
		<div class="comment-author">
			<div class="comment-info">
				...
				<span class="cmt-vote">
					<?php $c_name = 'fun_comment_vote_'.$comment->comment_ID;$cookie = isset($_COOKIE[$c_name])?$_COOKIE[$c_name]:'';?>
					<i class="fa fa-thumbs-o-up <?php if($cookie==1)echo 'voted'; ?>" title="<?php _e('顶一下','tinection'); ?>" data="<?php echo $comment->comment_ID; ?>" data-type="1" data-num="<?php echo (int)get_comment_meta($comment->comment_ID,'fun_comment_voteyes',true); ?>"><?php echo ' ['.(int)get_comment_meta($comment->comment_ID,'fun_comment_voteyes',true).']'; ?></i>
					<i class="fa fa-thumbs-o-down <?php if($cookie==2)echo 'voted'; ?>" title="<?php _e('踩一下','tinection'); ?>" data="<?php echo $comment->comment_ID; ?>" data-type="2" data-num="<?php echo (int)get_comment_meta($comment->comment_ID,'fun_comment_voteno',true); ?>"><?php echo ' ['.(int)get_comment_meta($comment->comment_ID,'fun_comment_voteno',true).']'; ?></i>
				</span>
...
<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
   <div id="div-comment-<?php comment_ID() ?>" class="comment-body">
      <?php $add_below = 'div-comment'; ?>
		<div class="comment-author"><?php $uid = get_user_by_email($comment->comment_author_email)->ID;echo fun_get_avatar($uid, 40, fun_get_avatar_type($uid)); ?>
...
/* 评论时给文章添加最近评论时间字段
/* --------------------------------- */
function fun_update_post_latest_reviewed_meta($comment_ID){
	$comment = get_comment($comment_ID);
	$post_id = $comment->comment_post_ID;
	update_post_meta($post_id,'fun_latest_reviewed',time());
}
add_action('comment_post','fun_update_post_latest_reviewed_meta');

/* 获取文章最新一条评论
/* --------------------- */
function fun_get_post_latest_comment($post_id){
	global $wpdb;
	$prefix = $wpdb->prefix;
