...
<div id="home-slider" class="container">
	<div id="flexslider" class="flexslider flexslider-nar">
		...
		<?php wp_reset_query(); else: echo '<p style="width:100%;position:absolute;top:50%;margin-top:-10px;text-align:center;font-weight:bold;color:#f00">'.__('请在后台设置中添加要显示幻灯的文章ID','xiapistudio').'</p>'; endif;?>
	</div>
	<div id="flexslider-right">
		<span class="slider-newest">
			<div class="slider-newest-title">
			<?php if($thelayout=='cms'){ ?>
				<h2><?php _e('最新动态','xiapistudio'); ?>
				<a href="<?php echo get_bloginfo('url').'/articles'; ?>" target="_blank" title="<?php _e('更多动态','xiapistudio'); ?>"><?php _e('更多','xiapistudio'); ?></a>
				<i class="fa fa-sitemap"></i>
				</h2>
			<?php }else{ ?>
				<h2><?php _e('热门动态','xiapistudio'); ?></h2>
			<?php } ?>
			</div>
			...
			<?php while ( $the_posts->have_posts() ) : $the_posts->the_post(); ?>
				...
				<article class="clr">
					...
					<span class="reviewer"><?php echo $comment->comment_author; ?></span><?php _e(' 评论于 ','xiapistudio'); ?><span class="date"><?php echo timeago($comment->comment_date_gmt); ?></span>
					<?php }else{ ?>
