...
?>
		<?php 
			...
			?>
			<!--h3 class="multi-border-hl"><span><?php _e('相关文章','xiapistudio'); ?></span></h3-->
			<ul>
			<?php 
		...
		<?php } else {?>
		<!--h3 class="multi-border-hl"><span><?php _e('推荐文章','xiapistudio'); ?></span></h3-->
		<ul>
			...
			<li>
				<div class="relpost-inner">
					...
					<?php $large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), 'large');?><?php if ( has_post_thumbnail()) { ?>
						<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="">
							<div class="thumb-img">
								<img <?php echo $lazy; ?>="<?php $imgsrc = $large_image_url[0]; echo fun_thumb_source($imgsrc,175,126,false); ?>" alt="<?php the_title(); ?>">
								<span><?php the_article_icon();?></span>
							</div>
						</a>
					<?php }?>
					<?php if ( !has_post_thumbnail() ) {  ?>	
						<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>" class="">
							<div class="thumb-img">
								<img <?php echo $lazy; ?>="<?php $imgsrc = catch_first_image(); echo fun_thumb_source($imgsrc,175,126,false); ?>">
								<span><?php the_article_icon();?></span>
							</div>
						</a>
						<?php }?>
